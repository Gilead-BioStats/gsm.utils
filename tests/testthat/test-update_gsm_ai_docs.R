test_that("update_gsm_ai_docs writes templates to target repo", {
  ...
  expect_true(file.exists(file.path(tmp, ".github", "ai", "ai_manifest.json")))
...
test_that("update_gsm_ai_docs check mode reports drift", {
  ...
  writeLines("# modified", file.path(tmp, ".github", "ai", "AGENTS.md"))

  report <- update_gsm_ai_docs(strPackageDir = tmp, mode = "check")

  expect_s3_class(report, "data.frame")
  expect_true(any(report$relative_path == file.path(".github", "ai", "AGENTS.md")))
  status <- report$status[report$relative_path == file.path(".github", "ai", "AGENTS.md")]
...
test_that("update_gsm_ai_docs dry_run does not overwrite existing files", {
  ...
  target <- file.path(tmp, ".github", "ai", "AGENTS.md")
...
test_that("update_gsm_ai_docs include filters synced files", {
  ...
  expect_true(file.exists(file.path(tmp, ".github", "ai", "AGENTS.md")))
  expect_true(file.exists(file.path(tmp, ".github", "ai", "ai_manifest.json")))
  expect_false(file.exists(file.path(tmp, ".github", "ai", "ECOSYSTEM.md")))
})
